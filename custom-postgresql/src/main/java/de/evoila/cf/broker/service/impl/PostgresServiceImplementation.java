...
public class PostgresServiceImplementation {
	...
	public void delete(String instanceId) throws SQLException {
        ...
        try {
            ...
            postgresSqlImplementation.executePreparedSelect("SELECT pg_terminate_backend(pg_stat_activity.pid) FROM "
            		+ "pg_stat_activity WHERE pg_stat_activity.datname = ? AND pid <> pg_backend_pid()", parameterMap);
            
            postgresSqlImplementation.executeUpdate("ALTER DATABASE \"" + instanceId + "\" OWNER TO \"" + currentUser + "\"");
            
            postgresSqlImplementation.executeUpdate("DROP DATABASE IF EXISTS \"" + instanceId + "\"");
            
            postgresSqlImplementation.executePreparedUpdate("DELETE FROM service WHERE serviceinstanceid=?", parameterMap);
        } catch (SQLException e) {
