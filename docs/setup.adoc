...
.TL;DR
[NOTE]
If you are buissy it can be sufficient to read "Module 1: Start Script" to beginn the installation.
...
The services are available at now are PostgreSQL, Redis, MongoDB, MySQL, RabbitMQ, Elasticsearch and Logstash.
Additionally we created deployment scripts for Monit for monitoring.
All deployment scripts are working on different environments.
In all deployment scripts is a build-in differentiation for the environment.
...
One speciality is the configuration of our services for a high availability after a failure.
Developed using the cattle principle the data is stored in a separate volume, which can backed up very easily.
This means, when a instance has a failure, a new service instance can be created and the stored volume can mounted to the new instance.
...
NOTE: For installation it is not nessasary to download any more scripts manualy. +
The start script downloads the template and this pulls all other nessasary scripts direct from the repository.

You have only to replace the Parameters with your own. Environment has to be replaced with "openstack" or "docker".
...
In the same way a service specific "monit control file" `[exampleservice]-monitrc` for the later following configuration of Monit will be loaded.
...
First this module prepares environment for installation with the following steps.
...
When the installation is successfully done this script can configure the service.
...
A divergence is required for the execution of processes in Docker as environment in mostly all scripts. The different start and stop commands are included in the current Central Template.
...
[NOTE]
Subtile distinctions between Openstack and Docker embedded direct in the code
...
Start command for a service for the environment Openstack is stored in the variable `$OPENSTACK_START`. The Stop command is stored in `$OPENSTACK_STOP`. The variables are defined in the `[exampleservice]-template.sh`.

==== Docker
For the environment Docker it is simmilar. The start command is stored in the variable `$DOCKER_START` and the Stop command in `$DOCKER_STOP`. The variables are also defined in the `[exampleservice]-template.sh`.
...
image::postgres_b.png[PostgreSQL-Logo,80]
...
Additional to the different start- and stop-commands on the environments Openstack and Docker Postgres uses different pathes to the configuration files.
...
Postgres accepts connections on port 5432 with the pgsql protokol.
...
Redis has a very good readable and commented configurationfile. Supplements can added in the `redis-configuration.sh`. Examles you can find on http://www.redis.io.
...
Redis accepts TCP connections on port port 6379.
We tested the login for an external desktop client successfully with Redis-Desktop-Manager.

Normaly Redis creates no PID file. While we use the PID file for monitoring with Monit we forced Redis to create one.
...
For MongoDB a parametrization in our scripts is avaible for database name, password, user and environment.
...
Additional to the different start- and stop-commands on the environments Openstack and Docker Mongo needs different configurations for the .
...
[CAUTION]
The repositories for the previous version are then no longer avaible on the old URL. This conducts that an installation is not possible.
...
On Rabbit-Instances a login to a web interface is possible with the rabbitmq-management-plugin (http://server-name:15672/). This plugin is activated on service instances deployed with our scripts by default.
...
So it is nessasary to check if java installation works correct to prevent a possible origin of trouble when somthing is changed in java.
...
A parametrization is build-in for user and password. In the standart configuration of the services the user is set to *monit* and the password is handled over the service-broker from the monitored service.
