...
The start script downloads the template which pulls all other necessary scripts directly from the repository.
...
The definition of the most needed variables are bundled in the central template.
...
At last Monit will be installed and executed for monitoring and service recovery (for detailed description read the topic <<monit>>).
...
icon:file[] `[exampleservice]-configuration.sh`
...
It is described in detail in the section "<<environment>>" of the documentation.
...
It will only be executed in case it is already installed.
...
==== Docker
For the environment Docker is similar. The start command is stored in the variable `$DOCKER_START` and the Stop command in `$DOCKER_STOP`. The variables are also defined in the `[exampleservice]-template.sh`.


[[servicespecific]]
=== Service specific

Generally all deployment scripts have the same structure, which means
you have to read the general description of the modules in the section <<Modularity>> first.


==== PostgreSQL


image::../assets/postgres_b.png[PostgreSQL-Logo,80]
...
image::../assets/redis_b.png[Redis-Logo,80]
...
image::../assets/mongodb_b.png[MongoDB-Logo,40]
...
It’s a Problem of the current Robomongo implementation, but work with other clients.


==== MySQL

image::../assets/mysql_b.png[MySQL-Logo,80]
...
image::../assets/rabbitmq_b.png[RabbitMQ-Logo,80]
...
image::../assets/elasticsearch_b.png[Elasticsearch-Logo,80]
...
So it is necessary to check if java installation works correctly to prevent a possible origin of trouble when something is changed in java.

==== Logstash

image::../assets/logstash_b.png[Logstash-Logo,80]
...
image::../assets/monit_b.png[Monit-Logo,80]
