...
import de.evoila.cf.broker.model.fixture.ServiceInstanceBindingFixture;
...
public class ServiceInstanceBindingControllerIntegrationTest {
	...
	private static final String BASE_PATH = "/v2/service_instances/"
			+ ServiceInstanceFixture.getServiceInstance().getId() + "/service_bindings";
	...
	// .thenReturn(instance);
	//
	// when(serviceInstanceBindingService.createServiceInstanceBinding(any(String.class),
	...
	// String body =
	// ServiceInstanceBindingFixture.getServiceInstanceBindingRequestJson();
	//
	// mockMvc.perform(
	...
	public void unknownServiceInstanceFailsBinding() throws Exception {
		...
		mockMvc.perform(put(url, binding.getId()).contentType(MediaType.APPLICATION_JSON).content(body)).andDo(print())
	...
	//
	// when(serviceInstanceService.getServiceInstance(any(String.class))).thenReturn(instance);
	//
	// when(serviceInstanceBindingService.createServiceInstanceBinding(any(String.class),
	...
	// binding.getId()).contentType(MediaType.APPLICATION_JSON).content(body))
	// .andExpect(status().isConflict()).andExpect(jsonPath("$.message",
	// containsString(binding.getId())));
	// }
	...
	public void invalidBindingRequestJson() throws Exception {
		...
		mockMvc.perform(put(url, binding.getId()).contentType(MediaType.APPLICATION_JSON).content(body))
				.andExpect(status().isUnprocessableEntity()).andExpect(jsonPath("$.message", containsString("foo")));
	}
	...
	// @Test
	// public void serviceInstanceBindingIsDeletedSuccessfully() throws
	// Exception {
	// ServiceInstance instance = ServiceInstanceFixture.getServiceInstance();
	// ServiceInstanceBinding binding =
	// ServiceInstanceBindingFixture.getServiceInstanceBinding();
	...
	// when(serviceInstanceService.getServiceInstance(any(String.class))).thenReturn(instance);
	//
	// when(serviceInstanceBindingService.deleteServiceInstanceBinding(any(String.class))).thenReturn(binding);
	//
	...
	// when(serviceInstanceService.getServiceInstance(any(String.class))).thenReturn(instance);
	//
	// when(serviceInstanceBindingService.deleteServiceInstanceBinding(any(String.class))).thenReturn(null);
	//
	// String url = BASE_PATH + "/" + binding.getId() + "?service_id=" +
	// instance.getServiceDefinitionId()
	// + "&plan_id=" + instance.getPlanId();
